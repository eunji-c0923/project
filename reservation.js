...
// ========================================
// 실시간 주차장 현황 로드 (요구사항 1)
// ========================================
async function loadLiveParkingStatus() {
  ...
  try {
    ...
    updateLiveParkingStatus(data);
    
    console.log('✅ 실시간 주차장 현황 로드 완료');
    return true;
  } catch (error) {
    console.error('❌ 실시간 주차장 현황 로드 실패:', error);
    return false;
  }
}

function updateLiveParkingStatus(data) {
  const { totalSlots, occupiedSlots, availableSlots, occupancyRate } = data;
  
  // 상태 숫자 업데이트
  const statusNumbers = document.querySelectorAll('.status-number');
  if (statusNumbers.length >= 4) {
    statusNumbers[0].textContent = totalSlots;
    statusNumbers[1].textContent = occupiedSlots;
    statusNumbers[2].textContent = availableSlots;
    statusNumbers[3].textContent = occupancyRate + '%';
  }
  
  // 개별 요소 업데이트
  updateElement('total-slots', totalSlots);
  updateElement('occupied-slots', occupiedSlots);
  updateElement('available-slots', availableSlots);
...
async function loadCurrentParkingStatus() {
  ...
  try {
    ...
    updateCurrentParkingStatus(data);
    
    console.log('✅ 현재 주차 상태 로드 완료');
    return true;
  } catch (error) {
...
function updateCurrentParkingStatus(data) {
  const { currentStatus, reservationStatus, history } = data;
  
  // 현재 상태 업데이트
  if (currentStatus && currentStatus.type === 'active') {
    updateCurrentActiveStatus(currentStatus);
  }
  
  // 예약 상태 업데이트
  if (reservationStatus && reservationStatus.type === 'reserved') {
    updateCurrentReservationStatus(reservationStatus);
  }
...
function updateCurrentActiveStatus(status) {
  const container = document.getElementById('current-active-status');
  if (!container) return;
  
  container.innerHTML = `
    <div class="active-status-card">
      <div class="status-header">
        <span class="status-badge active">주차중</span>
        <span class="slot-name">${status.slotName}</span>
      </div>
      <div class="status-details">
        <div class="detail-item">
          <span class="label">입차시간:</span>
          <span class="value">${status.entryTime}</span>
        </div>
        <div class="detail-item">
          <span class="label">경과시간:</span>
          <span class="value">${status.elapsedTime}</span>
        </div>
        <div class="detail-item">
          <span class="label">예상요금:</span>
          <span class="value">₩${status.estimatedFee.toLocaleString()}</span>
        </div>
      </div>
      <button onclick="exitParking()" class="btn-exit">출차하기</button>
    </div>
  `;
}

function updateCurrentReservationStatus(status) {
  const container = document.getElementById('current-reservation-status');
  if (!container) return;
  
  container.innerHTML = `
    <div class="reservation-status-card">
      <div class="status-header">
        <span class="status-badge reserved">예약중</span>
        <span class="slot-name">${status.slotName}</span>
      </div>
      <div class="status-details">
        <div class="detail-item">
          <span class="label">예약시간:</span>
          <span class="value">${status.reservationTime}</span>
        </div>
        <div class="detail-item">
          <span class="label">이용시간:</span>
          <span class="value">${status.duration}시간</span>
        </div>
        <div class="detail-item">
          <span class="label">예약요금:</span>
          <span class="value">₩${status.reservationFee.toLocaleString()}</span>
        </div>
      </div>
      <div class="reservation-actions">
        <button onclick="cancelCurrentReservation()" class="btn-cancel">예약취소</button>
        <button onclick="modifyCurrentReservation()" class="btn-modify">예약변경</button>
      </div>
    </div>
  `;
}
...
// ========================================
// 실시간 구역별 현황 로드 (요구사항 3)
// ========================================
async function loadRealtimeZoneStatus() {
  console.log('🏢 실시간 구역별 현황 로드 중...');
  
  try {
    const data = await apiRequest('/api/parking/realtime-status');
    if (!data || !data.zones) return false;
    
    updateRealtimeZoneStatus(data.zones);
    
    console.log('✅ 실시간 구역별 현황 로드 완료');
    return true;
  } catch (error) {
    console.error('❌ 실시간 구역별 현황 로드 실패:', error);
    return false;
  }
}

function updateRealtimeZoneStatus(zones) {
  ...
  zones.forEach(zone => {
    ...
    zoneCard.className = `zone-card zone-${zone.zoneCode.toLowerCase()}`;
    
    const usageRate = zone.usageRate;
    let statusClass = 'low';
    if (usageRate > 80) statusClass = 'high';
    else if (usageRate > 50) statusClass = 'medium';
    
    zoneCard.innerHTML = `
      <div class="zone-header">
        <span class="zone-name">${zone.zoneName}</span>
        <span class="zone-code">${zone.zoneCode}구역</span>
      </div>
      <div class="zone-stats">
        <div class="usage-display">
          <span class="used">${zone.used}</span>
          <span class="divider">/</span>
          <span class="total">${zone.total}</span>
        </div>
        <div class="usage-rate ${statusClass}">
          가용률 ${zone.usageRate}%
        </div>
      </div>
      <div class="zone-progress">
        <div class="progress-bar">
          <div class="progress-fill ${statusClass}" style="width: ${zone.usageRate}%"></div>
        </div>
      </div>
    `;
...
function updateAccountInfo(data) {
  ...
  // 포인트 정보 업데이트
  updateElement('current-points', point?.toLocaleString() + 'P');
  updateElement('expire-points', pointExpireNextMonth?.toLocaleString() + 'P');
  
  // 선불 잔액 정보 업데이트
  updateElement('prepaid-balance', '₩' + prepaidBalance?.toLocaleString());
  updateElement('last-charged', lastChargedAt);
  
  // 사용량 정보 업데이트
  updateElement('monthly-usage', '₩' + monthlyUsage?.toLocaleString());
  
  // 절약율 표시
  const savingsElement = document.getElementById('savings-rate');
  if (savingsElement && compareLastMonth !== undefined) {
    const isPositive = compareLastMonth > 0;
    savingsElement.textContent = `${isPositive ? '+' : ''}${compareLastMonth}%`;
    savingsElement.className = `savings-rate ${isPositive ? 'increase' : 'decrease'}`;
  }
}
...
// 이용 내역 로드 (요구사항 5)
// ========================================
async function loadUsageHistory(startDate, endDate, type = null, status = null) {
  console.log('📋 이용 내역 로드 중...');
...
function updateUsageHistory(data) {
  const { summary, history } = data;
  
  // 통계 요약 업데이트
  if (summary) {
    updateElement('total-count', summary.totalCount);
    updateElement('total-time', summary.totalTime + 'h');
    updateElement('total-paid', '₩' + summary.totalPaid.toLocaleString());
    updateElement('average-time', summary.averageTime + 'h');
  }
  
  // 이용 내역 목록 업데이트
  const historyContainer = document.getElementById('usage-history-list');
    ...
    historyItem.innerHTML = `
      ...
      <div class="history-details">
        <div class="detail-row">
          <span class="label">주차구역:</span>
          <span class="value">${item.slotName}</span>
        </div>
        <div class="detail-row">
          <span class="label">이용시간:</span>
          <span class="value">${item.duration} (${item.startTime} ~ ${item.endTime})</span>
        </div>
        <div class="detail-row">
          <span class="label">차량번호:</span>
          <span class="value">${item.carNumber}</span>
        </div>
        <div class="detail-row">
          <span class="label">결제금액:</span>
          <span class="value">₩${item.fee.toLocaleString()}</span>
        </div>
      </div>
    `;
...
function updateMembershipInfo(data) {
  ...
  // 멤버십 기본 정보 업데이트
  updateElement('membership-grade', membershipGrade);
  updateElement('membership-joined', joinedAt);
  updateElement('membership-usage-count', totalUsageCount);
  updateElement('membership-total-payment', '₩' + totalPayment.toLocaleString());
  updateElement('membership-discount-rate', discountRate + '%');
  
  // 멤버십 혜택 목록 업데이트
  const benefitsContainer = document.getElementById('membership-benefits');
  if (benefitsContainer && benefits) {
...
async function loadUserCars() {
  ...
  try {
    ...
    carSelects.forEach(select => {
      if (select.tagName === 'SELECT') {
        ...
        data.cars.forEach(car => {
          ...
          option.setAttribute('data-car-type', car.type || 'standard');
          option.setAttribute('data-car-size', car.size || 'medium');
    ...
    // 차량 목록 UI 업데이트
    updateCarListUI(data.cars);
    
    console.log('✅ 차량 정보 로드 완료', { count: data.cars.length });
    return true;
  } catch (error) {
...
function updateCarListUI(cars) {
  const carListContainer = document.getElementById('registered-cars-list');
  ...
  cars.forEach(car => {
    ...
    carItem.className = `car-item ${car.isPrimary ? 'primary' : ''}`;
    
    carItem.innerHTML = `
      <div class="car-info">
        <div class="car-number">${car.carNumber}</div>
        <div class="car-details">
          <span class="car-make-model">${car.manufacturer} ${car.model}</span>
      ...
      <div class="car-actions">
        ${!car.isPrimary ? `<button onclick="setPrimaryCar('${car.carNumber}')" class="btn-set-primary">주차용 설정</button>` : ''}
        <button onclick="editCar('${car.carNumber}')" class="btn-edit">수정</button>
        <button onclick="deleteCar('${car.carNumber}')" class="btn-delete">삭제</button>
      </div>
    `;
...
// 차량 관리 기능들
// ========================================
async function setPrimaryCar(carNumber) {
  try {
    ...
    if (response && response.success) {
      showToast('기본 차량이 설정되었습니다.', 'success');
      await loadUserCars();
    } else {
      showToast('기본 차량 설정에 실패했습니다.', 'error');
    }
  } catch (error) {
    console.error('❌ 기본 차량 설정 실패:', error);
    showToast('기본 차량 설정 중 오류가 발생했습니다.', 'error');
  }
}
...
async function deleteCar(carNumber) {
  ...
  try {
    ...
    showToast('차량 삭제 중 오류가 발생했습니다.', 'error');
  }
}

// ========================================
// 주차 구역 정보 로드 (기존 코드 수정)
// ========================================
...
// ========================================
// 기존 함수들 유지 (코드 길이 제한으로 인해 주요 부분만 표시)
// ========================================
...
window.loadLiveParkingStatus = loadLiveParkingStatus;
window.loadCurrentParkingStatus = loadCurrentParkingStatus;
window.loadRealtimeZoneStatus = loadRealtimeZoneStatus;
...
window.setPrimaryCar = setPrimaryCar;
